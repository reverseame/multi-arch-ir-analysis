"""List functions and lift them to LLIL using Binary Ninja headless.

Standalone CLI:
    python -m pipeline.backends.binja_llil_lift --binary <path> --outdir <dir>

python3 -m pipeline.backends.binja_llil_lift \
    --binary binaries/ls/coreutils-8.29_gcc-6.4.0_x86_32_O1_ls.elf --outdir results/ls_x86_32/binja_llil
"""
...
def lift_function(func, bv):
    ...
    ir_ops_counts = classify_il_function_ops(llil)
    ir_ast_counts = classify_il_function_ast(llil)
    ...
    num_escape_ops = classify_il_function_escape(llil)
    ...
    op_histogram = classify_il_function_op_histogram(llil)
    ...
    num_cfg_blocks, num_cfg_edges = il_function_cfg_counts(llil)
    return (
    ...
    with Timer() as timer:
        try:
            with load(str(binary_path)) as bv:
                ...
                write_whole_binary_dump(outdir, "whole_binary.llil.txt", whole_binary_chunks)
        except Exception as e:
            fatal_error = f"{type(e).__name__}: {e}"

    summary = write_summary(outdir, "binja_llil", binary_path, len(functions), lifted_records, timer.duration_s, fatal_error)
...
def main():
    parser = build_backend_arg_parser("List functions and lift to LLIL with Binary Ninja")
